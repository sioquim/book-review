...
import { upsertBook, insertReview } from "src/app/(index)/actions";
...
describe("upsertBook", () => {
  ...
  it("should successfully upsert a new book", async () => {
    ...
    const result = await upsertBook({ title: "Test Book", isbn: "1234567890" });
  ...
  it("should return an error if a book with the same title or ISBN already exists", async () => {
    ...
    const result = await upsertBook({ title: "Existing Book", isbn: "0987654321" });
  ...
  it("should return an error if there is a problem checking for existing books", async () => {
    mockSupabaseClient.maybeSingle.mockResolvedValue({ data: null, error: new Error("Database error") });
  ...
  it("should return an error if there is a problem inserting the book", async () => {
    mockSupabaseClient.maybeSingle.mockResolvedValue({ data: null, error: new Error("Insert error") });
...
describe("insertReview", () => {
  ...
  it("should successfully insert a new review", async () => {
    ...
    mockSupabaseClient.maybeSingle.mockResolvedValueOnce({ data: { id: 1 }, error: null });
  ...
  it("should return an error if there is a problem checking for existing reviews", async () => {
    mockSupabaseClient.maybeSingle.mockResolvedValueOnce({ data: null, error: new Error("Database error") });
    ...
    expect(result).toEqual({ error: "Error checking for existing reviews" });
  });

  it("should return an error if there is a problem inserting the review", async () => {
    mockSupabaseClient.maybeSingle.mockResolvedValueOnce({ data: null, error: null });
    mockSupabaseClient.maybeSingle.mockResolvedValueOnce({ data: null, error: new Error("Insert error") });
